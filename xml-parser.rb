#
# State Decoded Parser
# This takes the XML input from an export of Municode.com and converts it to State Decoded xml format, one file per law
# Originally developed by Will Prahl - http://github.com/kempchee
# Modified by Antonio Llanos - http://github.com/antoniorllanos
# Developed for Code for Miami, a Code for America Brigade
...
doc.xpath("book/level1").each do |part|
		if part_count==3
			depth=1
							part_name=part.xpath('subtitle[1]')[0].content.strip.gsub(/ /,"_");
							o=File.open("import-data/#{part_name}.xml","w+")
							o.write("<?xml version='1.0' encoding='utf-8'?>\n")
							o.write("<law>\n<structure>\n")
							o.write("<unit label='part' identifier='PART 2' order_by='#{order_structure(part_count)}' level='#{(depth).to_s}'>#{part.xpath('breadcrumbs/crumb[2]/caption')[0].content.strip}</unit>\n")
							o.write("</structure>\n")
							#this doesn't work, as section number must be unique.  If Chapter_XX, parse out XX, otherwise, first letter of each
							o.write("<section_number>abc</section_number>\n")
							o.write("<catch_line>#{part.xpath('subtitle[1]')[0].content.gsub(/ & /," &amp; ").strip}</catch_line>\n")
				...
				if(!chapter.xpath("level3").empty?)
					...
					chapter.xpath("level3").each do |article|
						...
						article.xpath("section").each do |section|
							...
							o.write("<catch_line>#{section.xpath('subtitle[1]')[0].content.gsub(/ & /," &amp; ").strip}</catch_line>\n")
							o.write("<order_by>#{order_by(global_section_count)}</order_by>\n")
							o.write("<text>")
						...
						article.xpath("level4").each do |article_lvl4| 
						 #depth=4
						 section_count=1
						 
						  article_lvl4.xpath("section").each do |section|
							depth=4
							puts section.xpath('title[1]')[0].content.strip.split(" ")[-1].gsub(/\.$/,"").gsub(/ /,"_")
							...
							o.write("<unit label='chapter' identifier='#{order_structure(chapter_count)}' order_by='#{order_structure(chapter_count)}' level='#{(depth-2).to_s}'>#{chapter.xpath('breadcrumbs/crumb[3]/caption')[0].content.strip}</unit>\n")
							o.write("<unit label='article' identifier='#{order_structure(article_count)}' order_by='#{order_structure(article_count)}' level='#{(depth-1).to_s}'>#{article.xpath('breadcrumbs/crumb[4]/caption')[0].content.strip}</unit>\n")
							o.write("<unit label='division' identifier='#{order_structure(division)}' order_by='#{order_structure(division)}' level='#{(depth).to_s}'>#{article_lvl4.xpath('breadcrumbs/crumb[5]/caption')[0].content.strip}</unit>\n")
							o.write("</structure>\n")
							o.write("<section_number>#{section_name}</section_number>\n")
					...
					if(!chapter.xpath("section").empty?)
						...
						chapter.xpath("section").each do |section|
								...
								depth=2
								puts chapter.xpath("title[1]")
                                puts chapter.xpath("title[1]")[0].content.empty?
								if chapter.xpath("title[1]")[0].content.empty?
									chapter_name=chapter.xpath('subtitle[1]')[0].content.strip.gsub(/ /,'_').gsub(/ & /," &amp; ")
                                    
								else 
									chapter_name=chapter.xpath('title[1]')[0].content.strip.gsub(/ /,'_')+"_"+chapter.xpath('subtitle[1]')[0].content.strip.gsub(/ /,'_').gsub(/ & /,"&amp;")
